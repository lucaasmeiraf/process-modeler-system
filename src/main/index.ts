...
function createWindow(): void {
  ...
  // Set CSP to allow Supabase and other external resources
  mainWindow.webContents.session.webRequest.onHeadersReceived((details, callback) => {
    // Only apply CSP to local files, not external resources
    if (details.url.startsWith('http://localhost') || details.url.startsWith('file://')) {
      callback({
        responseHeaders: {
          ...details.responseHeaders,
          'Content-Security-Policy': [
            "default-src 'self' https://*.supabase.co http://localhost:*; " +
      ...
      // Don't modify headers for external requests
